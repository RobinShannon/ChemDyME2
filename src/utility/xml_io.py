...
def writeTemplate(temperature,pressure):

def writeTSXML(React, path):
    spinMult = 2

    cml = tl.getCML(React.TS, 'TS_' + React.ReacName + '_' + React.ProdName)
    ...
    tl.prettyPrint(dom, path)


def writeMinXML(React, path, isReactant, isBi):
    spinMult = 2
    if isReactant and isBi:
        cml = tl.getCML(React.biReac, React.biReacName)
        spinMult = tl.getSpinMult(React.biReac, React.biReacName)
        ...
        cml = tl.Tools.getCML(React.Prod, React.ProdName)
        spinMult = tl.Tools.getSpinMult(React.Prod, React.ProdName)
...
def writeReactionXML(React, path, printTS2):
    ...
    if React.is_bimol_reac == False and React.is_bimol_prod == False:
        ...
        if React.barrierlessReaction:
            ...
            act = float(((React.forwardBarrier - React.energyDictionary[symb]) * 96.45) - (
                        (React.reactantEnergy - React.energyDictionary[symb]) * 96.45))
        if act < 0:
            act = 0
        React.barrierlessReaction = True
    if React.is_bimol_prod == False:
        ...
        if React.barrierlessReaction:
            ...
            React.barrierlessReaction = True
            act = float(((React.forwardBarrier - React.energyDictionary[symb]) * 96.45) - (
                        (React.productEnergy - React.energyDictionary[symb]) * 96.45))
        if act < 0:
            act = 0
    else:
