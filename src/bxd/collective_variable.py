...
class CollectiveVariable(object):
    ...
    get_s_from_array: Generate s from an array object rather than an atoms object. Only implemented for certain derived
                      classes
    ...
    def get_s_from_array(self, arr):
        pass


class PrincipalCoordinates(CollectiveVariable):
    """
    Collective variable consists of an arbitrary number of principal coordinates, each of which  is a linear
    ...
    :param pc_array: List of arrays,each containing the coefficients (column 0) and corresponding atom indicies
                    (columns 1 and 2) for a single PC. There is one array for each PC considered. This list of array
                    is generated by the utility.DimensionalityReduction class
    :param number_of_elements: Point at which to truncate linear combination in each PC
    """

    def __init__(self, pc_array, number_of_elements):
        super().__init__()
        ...
        # for each PC in the list separate out the 3D array into coefficients and the corresponding atom indicies
        for f in pc_array:
    ...
    def get_s(self, mol):
        """
        Evaluate the n dimensional vector of PC's. Util contains a Cythonized function for calculating the value of
    ...
    def read_principal_components(self, arr):
        ...
        :param arr: 2D array containing coefficients and followed by atom indicies
    ...
    def get_delta(self, mol, n):
        ...
        """
        return cython_utils.get_delta(mol, n, self.indicies, self.coefficients)
...
class CartesianPrincipalCoordinates(CollectiveVariable):
    """
    Collective variable is a linear combination of cartesian coordinates.
    :param number_of_pcs: Number of principal coordinates
    ...
    def __init__(self, number_of_pcs, input_prefix, file=True, numpy_array=None):
        ...
        for i in range(0, number_of_pcs):
            if file:
                inp = input_prefix + str(i) + '.txt'
                array = np.loadtxt(inp)
            else:
                array=numpy_array
    ...
    def get_s(self, mol):
        """
        Evaluate the n dimesnional vector of PC's.
        :param mol: ASE atoms object with current cartesian coordinates
        :return: Numpy array of floats corresponding to the value of each PC at the current geometry
        """
        ...
        for i in range(0, len(mol)):
            ...
            for j, d in enumerate(self.distances):
                ...
                if mol.get_distance(int(d[0]), int(d[1])) != 0:
                    first_term = (1 / (2 * mol.get_distance(int(d[0]), int(d[1]))))
                else:
                    first_term = 0
                norm = n[j]
                # if atom i is the first atom in bond j then add component to the derivative based upon chain rule
                # differentiation
                if d[0] == i:
                    index = int(d[1])
                    constraint[i][0] += first_term * 2 * (pos[i][0] - pos[index][0]) * norm
                    constraint[i][1] += first_term * 2 * (pos[i][1] - pos[index][1]) * norm
                    constraint[i][2] += first_term * 2 * (pos[i][2] - pos[index][2]) * norm
                # alternate formula for case where i is the second atom
                if d[1] == i:
                    index = int(d[0])
                    constraint[i][0] += first_term * 2 * (pos[index][0] - pos[i][0]) * -1 * norm
                    constraint[i][1] += first_term * 2 * (pos[index][1] - pos[i][1]) * -1 * norm
                    constraint[i][2] += first_term * 2 * (pos[index][2] - pos[i][2]) * -1 * norm
        return constraint
